...
static_assert(std::is_same_v<std::index_sequence<0, 2>,
...
auto tied(const TestStruct& t) { return std::tie(t.i, t.j, t.k); }
...
TEST_CASE("make_matcher: matching tuple with no args")
{
    ...
    SECTION("one case, matching pattern, executes function")
    {
        ...
        auto match_ = make_matcher(case_{_, _, _} |= [&] { executed = true; });
    ...
    SECTION("two cases, one matching pattern, executes function")
    {
        ...
        auto match_ = make_matcher(case_{2, 2, 1} |= [] {},
    ...
    SECTION("two cases, no matching pattern, function not executed")
    {
        ...
        auto match_ = make_matcher(case_{2, 2, 1} |= [&] { executed = true; },
                                   case_{1, 2, 1} |= [&] { executed = true; });
    ...
    {
        auto match_ = make_matcher(case_{_, _, _} |= [] { return 50; });
...
TEST_CASE("make_matcher: matching tuple with args")
{
    ...
    SECTION("one case, matching pattern, executes function with correct arg")
    {
        ...
        auto match_ = make_matcher(case_{_, arg, _} |= [&](auto arg) {
            executed_with_correct_arg = (arg == 2);
        });
        match_(TestStruct{1, 2, 3});
    ...
    SECTION("one case, matching pattern, executes function with multiple args "
            "with correct args")
    {
        bool executed_with_correct_args = false;

        auto match_ =
            make_matcher(case_{_, arg, arg} |= [&](auto arg1, auto arg2) {
...
TEST_CASE("make_matcher: matching range with args")
{
            ...
            "with correct args")
    {
        ...
        auto match_ =
            make_matcher(case_{_, arg, arg} |= [&](auto arg1, auto arg2) {
                executed_with_correct_args = (arg1 == 2) && (arg2 == 1);
            });
        match_(std::vector<int>{1, 2, 3} | reversed);
...
TEST_CASE("match")
{
    ...
    SECTION("one case, matching pattern, executes function with correct arg")
    {
        ...
        match(std::vector<int>{1, 2, 3})(case_{_, arg, _} |= [&](auto arg) {
            executed_with_correct_arg = (arg == 2);
        });
    ...
    SECTION("non-default return possible with default case")
    {
        ...
        static_assert(
            detail::is_any_pattern_v<
                std::decay_t<std::decay_t<decltype(default_)>::pattern_type>>);
        static_assert(
            detail::has_any_pattern_v<decltype(operator|=(default_, d))>);

        auto result = match(std::vector<int>{1, 2, 3})(
            case_{_, _, _} |= [&] { return non_default{10}; },
            default_ |= [] { return non_default{5}; }); // should compile
        std::ignore = result;
    }
    ...
    SECTION("supports returning reference")
    {
        const int i = 666;
        const auto& result =
            match(std::make_tuple(10))(case_{_} |= [&]() -> decltype(auto) {
                return std::forward<const int>(i);
            });
        CHECK(&i == &result);
    }
}
