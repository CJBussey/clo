...
TEST_CASE("pattern: operator==(pattern, tuple)")
{
    ...
    SECTION("pattern<T> and tuple<T> with at least one different value returns "
            "false")
    {
        auto sut   = pattern{666, 666};
...
TEST_CASE("pattern: operator==(pattern, array)")
{
    ...
    SECTION("pattern<val> and array<T> with at least one different value "
            "returns false")
    {
        auto sut   = pattern{666, 666};
...
TEST_CASE("pattern: operator==(pattern, vector)")
{
    ...
    SECTION("pattern<val> and vector<T> with at least one different value "
            "returns false")
    {
        auto sut   = pattern{666, 666};
...
auto tied(const TestStruct& s) { return std::tie(s.i, s.j, s.k); }
...
TEST_CASE("pattern: operator==(pattern, struct)")
{
    ...
    SECTION("pattern<val> and struct with at least one different values "
            "returns false")
    {
...
TEST_CASE("nested patterns")
{
    ...
    SECTION(
        "pattern<pattern<val>> returns true if matching two dimensional vector")
    {
